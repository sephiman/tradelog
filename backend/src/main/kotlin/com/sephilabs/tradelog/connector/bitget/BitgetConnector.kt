...
/**
 * Bitget USDT-M futures on the UTA (Unified Trading Account) v3 API. A classic-account key cannot
 * call these endpoints and will be rejected — that is Bitget's account split, not a setting here.
 */
@Component
class BitgetConnector(
    props: AppProperties,
    mapper: ObjectMapper,
) : ClosedPositionConnector(props.connectors.bitget, mapper, props) {
    ...
    override fun checkEnvelope(root: JsonNode, path: String) {
        val code = root.path("code").asText("00000")
    ...
    private fun fetchWindow(creds: ExchangeCredentials, start: Instant, end: Instant): List<JsonNode> =
        pageThrough<String>(maxPages = MAX_PAGES) { cursor ->
            ...
            // Bitget returns the continuation as a cursor on the envelope; a short page ends the walk.
            val next = root.path("data").text(FIELD_CURSOR)
                ?: rows.lastOrNull()?.text(FIELD_CURSOR)
            Page(rows, next?.takeIf { rows.size >= LIMIT })
        }

    /** Test seam: map a raw history-position body. */
    ...
    override fun mapPosition(row: JsonNode): Mapped<PositionRecord> {
        ...
        // `cumRealisedPnl` is gross (fees and funding excluded); both fee components and funding are
    ...
    private companion object {
        const val PATH_HISTORY = "/api/v3/position/history-position"
        const val CATEGORY = "USDT-FUTURES"
